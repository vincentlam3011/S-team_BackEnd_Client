...
module.exports = {
    ...
    getJobById: (id) => {
        ...
        let query3 = `select  distinct  j.id_job,app.proposed_price,u.fullname,u.id_user,u.dial,u.email, app.id_status
            from jobs as j 
    ...
    getJobsByApplicantId: (id_user, status) => {
        if(status === 9) {
            ...
            where j.id_job = a.id_job and a.id_user = ${id_user} and j.employer = u.id_user and j.area_province = p.id_province and j.area_district = d.id_district 
            group by j.id_job`);
        }
        else if (status === 1) {
            return db.query(`
            ...
            return db.query(`
            select j.*, a.id_applicant, ap.id_status as applicant_status, u.fullname, u.email, u.dial, u.avatarImg, p.name as province, d.name as district, jp.deadline as deadline, jt.start_date as start_date, jt.end_date as end_date, jt.salary_type
    ...
    getJobsByEmployerId: (id_user, status) => {
        ...
        if(status === 9) {
            return db.query(`
            select j.*, jp.deadline as deadline, jt.start_date as start_date, jt.end_date as end_date, jt.salary_type, p.name as province, d.name as district
            from ((jobs as j left join jobs_production as jp on j.id_job = jp.id_job) left join jobs_temporal as jt on j.id_job = jt.id_job), provinces as p, districts as d
